...
def OS2OS_kp(query_meta,database_meta,matchScores,query_center,databaseShape,probeim,dbim,dbimname): #metadata is in matrix form, with rows as (x,y,scale,angle)
    showResults = False
...
def OS2OS(img1,img1Rect,img2,autoCenter=True):
    if not autoCenter:
    ...
    for flip in [False,True]:
        ...
        voteCoords = (OS2OS_kp(queryMeta,databaseMeta,scores,roiCenter,img2.shape,None,None,"")-nsize/2).astype(np.int)
    ...
    for m in goodMatches:
        ...
        #
        #     plt.imshow(voteMap)
        #     plt.savefig('./OS2OS_gif/threshold/'+str(i).zfill(3)+'.png')
        # print(i)
        i+=1
    voteMax = voteMap.max()
    ...
    #     plt.imshow(t)
    #     plt.savefig('./OS2OS_gif/threshold/'+str(i+500).zfill(3)+'.png')
    #     plt.close()
    #     print(i)
    # plt.figure()
    # plt.imshow(voteMap)
...
# OS2OS(im1,(2833,1183,704,1028),im2)
...
# im1 = cv.imread(im1path)
# im2 = cv.imread(im2path)
# OS2OS(im1,(0,0,im2.shape[0],im2.shape[1]),im2)


def runOS2OSForFileList(probeFile,fileList,outfolder,randomFiles=[]):
    try:
            ...
            if imgFile.endswith('.png') or imgFile.endswith('.jpg'):
                ...
                if rImg is None or rImg.shape[0] <= 0 or rImg.shape[1] <= 0:
                    ...
                    if 'g121_bbeCOhh.png' in imgFile:
                        ac=False
                    votemap,center,score,time = OS2OS(probeImg,rect,rImg,autoCenter=ac)
                    times.append(time)
...
def runOS2OSForFolder(folder,outfolder):
    if os.path.isdir(folder):
        ...
        runOS2OSForFileList(probeFile, fileList, outfolder)

def runOS2OSForJSON(jsonFile,outfolder):
    #folderPath = 'scratch2/google-landmarks-dataset/train4/'
    folderPath = '/scratch2/Reddit_Prov_Dataset_v5/jsons'
    ...
    runOS2OSForFileList(probeFile,fileList[1:],outfolder,randomFiles=randtosend)
...
if __name__ == "__main__":
    ...
    for j in allJsons:
        ...
        #runOS2OSForJSON(jsonFile,outFolder)
        runOS2OSForFolder(jsonFile,outFolder)